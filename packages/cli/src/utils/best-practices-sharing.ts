...
export function generateTypeScriptBestPractices(config: BestPracticesConfig): string {
  ...
  code += '    practice.votes = practice.votes.filter((v: unknown) => v.userId !== userId);\n\n';
  code += '    // Add new vote\n';
  ...
  code += '    const approves = practice.votes.filter((v: unknown) => v.type === \'approve\').length;\n';
  ...
  code += '  }\n\n';
  code += '  addComment(libraryId: string, practiceId: string, comment: unknown): void {\n';
  code += '    const library = this.libraries.get(libraryId);\n';
  code += '    if (!library) return;\n\n';
  code += '    const practice = library.practices.find((p: BestPractice) => p.id === practiceId);\n';
  code += '    if (!practice) return;\n\n';
  ...
  code += '  }\n\n';
  code += '  getCommunityMetrics(libraryId: string): unknown {\n';
  code += '    const library = this.libraries.get(libraryId);\n';
  code += '    if (!library) {\n';
