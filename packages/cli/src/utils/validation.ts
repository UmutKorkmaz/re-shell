...
export function validateGlobalConfig(config: Record<string, unknown>): ValidationResult {
  return configValidator.validateConfiguration(config, GLOBAL_CONFIG_SCHEMA, 'global');
}

export function validateProjectConfig(config: Record<string, unknown>): ValidationResult {
