...
export function registerWorkspaceGroup(program: Command): void {
  ...
  workspace
    .command('summary')
    .description('Emit an aggregate workspace summary (root, packageManager, workspaces, graph, health)')
    .option('--json', 'Emit machine-readable JSON envelope to stdout')
    .action(
      createAsyncCommand(async options => {
        // In --json mode stdout must stay pure JSON, so suppress the spinner.
        const spinner = options.json
          ? undefined
          : createSpinner('Building workspace summary...').start();
        if (spinner) {
        ...
        await withTimeout(async () => {
          await produceWorkspaceSummary({ ...options, spinner });
        }, 60000); // 1 minute timeout

        if (spinner) {
          spinner.succeed(chalk.green('Workspace summary generated successfully!'));
        }
      })
    );
  ...
  workspace
    ...
    .action(
      createAsyncCommand(async options => {
        const spinner = options.json ? undefined : createSpinner('Loading workspaces...').start();
        if (spinner) {
          processManager.addCleanup(() => spinner.stop());
          flushOutput();
        }
  ...
  workspace
    ...
    .action(
      createAsyncCommand(async options => {
        ...
        // When the graph is emitted as JSON to stdout, stdout must stay pure
        // JSON, so suppress the spinner entirely (mirrors --json gating).
        const jsonOutput = format === 'json' && !options.output;
        const spinner = jsonOutput
          ? undefined
          : createSpinner('Generating workspace graph...').start();
        if (spinner) {
          processManager.addCleanup(() => spinner.stop());
          flushOutput();
        }
        ...
        await withTimeout(async () => {
          await generateWorkspaceGraph({ ...options, format, spinner });
        }, 60000); // 1 minute timeout
  ...
  workspace
    .command('migrate-monorepo')
    .description('Migrate an Nx or Turborepo workspace into a re-shell.workspaces.yaml (v2)')
    .requiredOption('--from <tool>', 'Source monorepo tool (nx, turbo)')
    .option('--output <path>', 'Output path for the generated workspace YAML')
    .option('--dry-run', 'Print the would-be YAML without writing any file')
    .option('--json', 'Emit a single JSON envelope { detected, yaml } to stdout')
    .action(
      createAsyncCommand(async options => {
        ...
        if (options.json) {
          ...
          try {
            await withTimeout(async () => {
              if (from !== 'nx' && from !== 'turbo') {
                fail(
                  'MONOREPO_MIGRATE_ERROR',
                  `Unsupported --from value: ${options.from}. Use 'nx' or 'turbo'.`
                );
                return;
              }
              try {
                const { migrateMonorepo } = await import('../commands/migrate-monorepo');
                const result = await migrateMonorepo({
                  source: from,
                ...
                if (options.output && !options.dryRun) {
                  ...
                  await fsExtra.writeFile(options.output, result.yaml);
                }

                ok({ detected: result.detected, yaml: result.yaml });
              } catch (error: unknown) {
                const message =
                  error instanceof Error ? error.message : 'Unknown migration error';
                fail('MONOREPO_MIGRATE_ERROR', message);
              }
            }, 120000);
          } finally {
            restore();
        ...
        const spinner = createSpinner(`Migrating ${from} workspace...`).start();
        processManager.addCleanup(() => spinner.stop());
        flushOutput();

        await withTimeout(async () => {
          if (from !== 'nx' && from !== 'turbo') {
            spinner.fail(chalk.red(`Unsupported --from value: ${options.from}. Use 'nx' or 'turbo'.`));
            process.exitCode = 1;
            return;
          }
          ...
          if (options.dryRun || !options.output) {
            spinner.succeed(
              chalk.green(`Detected ${result.detected.length} service(s) from ${from}:`)
            );
            console.log();
            ...
            spinner.succeed(
              chalk.green(
                `Migrated ${result.detected.length} service(s) → ${options.output}`
              )
            );
          }
        }, 120000); // 2 minute timeout
  ...
  workspace.addCommand(policyGroup);

  // === DEPENDENCY DRIFT (P9-G2) ===
  workspace
    .command('drift')
