...
export class ConfigurationValidator {
  ...
  validateConfiguration(config: Record<string, unknown>, schema: ValidationRule[], context = ''): ValidationResult {
    this.reset();
  ...
  private validateField(value: unknown, rule: ValidationRule, fieldPath: string, fullConfig: Record<string, unknown>): void {
    switch (rule.type) {
      case 'required':
  ...
  private validateCustom(value: unknown, rule: ValidationRule, fieldPath: string, fullConfig: Record<string, unknown>): void {
  ...
  private validateGlobalContextual(config: Record<string, unknown>): void {
    ...
    const marketplace = plugins?.marketplace as Record<string, unknown> | undefined;
  ...
  private validateProjectContextual(config: Record<string, unknown>): void {
    ...
    if (dev?.port) {
      ...
      if (commonPorts.includes(port)) {
        this.addWarning({
          field: 'dev.port',
          message: `Port ${port} is commonly used and may cause conflicts`,
...
export function validateGlobalConfig(config: object): ValidationResult {
  return configValidator.validateConfiguration(config as Record<string, unknown>, GLOBAL_CONFIG_SCHEMA, 'global');
}

export function validateProjectConfig(config: object): ValidationResult {
  return configValidator.validateConfiguration(config as Record<string, unknown>, PROJECT_CONFIG_SCHEMA, 'project');
}
