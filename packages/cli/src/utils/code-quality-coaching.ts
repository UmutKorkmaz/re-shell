...
export function generateTypeScriptCodeQualityCoaching(config: CodeQualityCoachingConfig): string {
  ...
  code += '    return issues;\n';
  code += '  }\n\n';
  code += '  generateRecommendations(developerId: string): unknown[] {\n';
  code += '    const profile = this.profiles.get(developerId);\n';
  code += '    if (!profile) {\n';
  code += '      throw new Error(`Profile not found: ${developerId}`);\n';
  code += '    }\n\n';
  ...
  code += '  }\n\n';
  code += '  scheduleCoachingSession(developerId: string, coachId: string): unknown {\n';
  code += '    const profile = this.profiles.get(developerId);\n';
  code += '    if (!profile) {\n';
  code += '      throw new Error(`Profile not found: ${developerId}`);\n';
  code += '    }\n\n';
  ...
  code += '        .filter((m: unknown) => m.score < 70)\n';
  code += '        .map((m: unknown) => m.metric),\n';
  code += '      notes: \'\',\n';
  code += '    };\n\n';
  code += '    this.emit(\'session-scheduled\', session);\n';
  code += '    return session;\n';
  code += '  }\n\n';
  code += '  getDeveloperProgress(developerId: string): unknown {\n';
  code += '    const profile = this.profiles.get(developerId);\n';
  code += '    if (!profile) {\n';
  code += '      throw new Error(`Profile not found: ${developerId}`);\n';
  code += '    }\n\n';
  code += '    const avgScore = profile.metrics.reduce((sum: number, m: unknown) => sum + m.score, 0) / profile.metrics.length;\n';
  code += '    const decliningMetrics = profile.metrics.filter((m: unknown) => m.trend === \'declining\').length;\n\n';
  code += '    return {\n';
  code += '      developerName: profile.developerName,\n';
  code += '      averageScore: Math.round(avgScore),\n';
  code += '      totalIssues: profile.issues.length,\n';
  code += '      activeRecommendations: profile.recommendations.filter((r: unknown) => r.status === \'pending\').length,\n';
