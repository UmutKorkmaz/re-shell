...
/**
 * Serializable, machine-friendly view of a backend template suitable for
 * --json output. Strips heavy fields (file contents, dependency maps) and
 * exposes only the metadata consumers need to list and inspect templates.
 */
...
/**
 * Map a registry entry to its serializable summary. Keeps the shape stable so
 * the templates group can emit it directly via ok(...).
 */
export function toTemplateSummary(t: BackendTemplate): TemplateSummary {
  return {
    id: t.id,
    name: t.name,
    ...
    features: t.features,
    port: t.port,
    fileCount: t.files ? Object.keys(t.files).length : undefined,
  };
}
