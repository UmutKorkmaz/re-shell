 ...
 */
async function safePrompt<T = Record<string, unknown>>(questions: unknown[], options: { debug?: boolean } = {}): Promise<T> {
  if (options.debug) {
...
export async function initMonorepo(name: string, options: InitOptions = {}): Promise<void> {
  ...
  if (!forceNonInteractive) {
    ...
    const promptsToRun: Record<string, unknown>[] = [];
  ...
  const finalOptions = {
    packageManager: options.packageManager || (responses.packageManager as string) || 'pnpm',
    ...
    template: options.template || (responses.template as string) || 'blank',
    typescript: responses.typescript !== undefined ? (responses.typescript as boolean) : true,
...
async function applyTemplate(projectPath: string, template: string, options: Record<string, unknown>): Promise<void> {
  const templateConfig = TEMPLATES[template as keyof typeof TEMPLATES];
  if (!templateConfig) return;

  const structure = (options.structure as Record<string, unknown>) || {};
  const appsDir = path.join(projectPath, (structure.apps as string) || 'apps');
  const packagesDir = path.join(projectPath, (structure.packages as string) || 'packages');
