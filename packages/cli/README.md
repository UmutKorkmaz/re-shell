...
# Launch the local shadcn web dashboard (token-authed hub on 127.0.0.1)
re-shell ui
...
Every introspection command supports a `--json` flag that emits a single
machine-readable envelope (see [`--json` output envelope](#-json-output-envelope)):

```bash
# Project / environment health diagnostics
re-shell doctor
...
# Discover and inspect the framework template registry
re-shell templates list --json
re-shell templates list --language typescript --framework express
re-shell templates show express --json

# Introspect the CLI's own command tree as a catalog
re-shell commands list --json

# Install shell completion scripts (bash/zsh)
...
// error
{ "ok": false, "error": { "code": "TEMPLATE_NOT_FOUND", "message": "…", "details": { } }, "warnings": [] }
```

`code` is drawn from a closed `ErrorCode` set. In `--json` mode the CLI emits
exactly one parseable document on stdout — no spinner, banner, or color noise.
...
re-shell create user-service --template nestjs --database postgresql
...
| `create` | Create service/frontend | `re-shell create api --template express` |
...
re-shell create order-service --template express --database postgresql
...
re-shell create transaction-service --template actix-web --performance optimized
...
Re-Shell CLI is open source software released under the **Apache-2.0 License**. This means you can:
