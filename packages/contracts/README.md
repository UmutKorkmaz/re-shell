...
```bash
pnpm add @umutkorkmaz/contracts
```
...
Everything is re-exported from the package root (`@umutkorkmaz/contracts`).
...
```ts
import type { JsonResponse, JsonSuccess, JsonError } from '@umutkorkmaz/contracts';
...
```ts
import { errorCodeSchema } from '@umutkorkmaz/contracts';
...
```bash
pnpm --filter @umutkorkmaz/contracts build      # tsc -> dist/
pnpm --filter @umutkorkmaz/contracts typecheck
```
