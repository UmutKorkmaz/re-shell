 ...
 *      from @umutkorkmaz/contracts — the canonical, zod-backed wire envelope.
 *
...
describe('contract conformance: --json envelope + data shapes', () => {
  beforeAll(() => {
    ...
    if (!fs.existsSync(CLI_PATH)) {
      throw new Error(
        `Built CLI not found at ${CLI_PATH}. Run \`pnpm --filter @umutkorkmaz/re-shell-cli run build\` first.`
      );
    }
  });
