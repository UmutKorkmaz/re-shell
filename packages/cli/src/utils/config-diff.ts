...
export class ConfigDiffer {
  ...
  async applyDiff(base: Record<string, any>, diff: ConfigDiff): Promise<unknown> {
  ...
  private deepDiff(left: unknown, right: unknown, currentPath: string, changes: DiffEntry[]): void {
    ...
    if (leftType === 'object') {
      this.diffObjects(left as Record<string, unknown>, right as Record<string, unknown>, currentPath, changes);
  ...
  // Compare objects
  private diffObjects(left: Record<string, unknown>, right: Record<string, unknown>, currentPath: string, changes: DiffEntry[]): void {
  ...
  private deepMerge(
    ...
    // Handle different types
    if (leftType === 'object') {
      return this.mergeObjects(left as Record<string, unknown>, right as Record<string, unknown>, currentPath, strategy, conflicts, warnings);
    } else if (leftType === 'array') {
    ...
    switch (strategy.arrayMerge) {
      ...
      case 'custom':
        if (strategy.customResolver) {
          return strategy.customResolver(left, right, currentPath) as unknown[];
        }
  ...
  private setValueAtPath(obj: Record<string, any>, path: string, value: unknown): void {
  ...
  private removeValueAtPath(obj: Record<string, any>, path: string): void {
    const keys = path.split('.');
    const lastKey = keys.pop()!;
