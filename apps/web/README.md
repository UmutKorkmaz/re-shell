...
[`@umutkorkmaz/ui`](../../packages/ui) — there is **no Web Components layer**.
...
- **Transport**: SSE `/events` for streamed state, WS `/jobs` for live job output.
  ...
  [`@umutkorkmaz/contracts`](../../packages/contracts).
...
pnpm --filter @umutkorkmaz/ui-web dev

# Production build (app + bundled hub-server)
pnpm --filter @umutkorkmaz/ui-web build

# Tests (hub + UI suites) and typecheck
pnpm --filter @umutkorkmaz/ui-web test
pnpm --filter @umutkorkmaz/ui-web typecheck
```
...
```ts
import { WorkspaceSummaryPanel } from '@umutkorkmaz/ui';
