...
### Changed
- Point `homepage` at the new documentation site (https://umutkorkmaz.github.io/re-shell/) so it shows as the package link on npm.

### Fixed
- Update `bugs` URL to the live `UmutKorkmaz/re-shell` repository (the old `re-shell-cli` repo link was dead).
...
- rebranded the npm package from `@re-shell/cli` to `re-shell-cli`
